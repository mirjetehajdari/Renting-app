            ...
            <tbody>
                @foreach ($my_rents as $key => $rent)
                    <tr>
                        ...
                        {{-- <td class="border">{{ $rent->bike_id }}</td> --}}
                        <td class="border">{{ $rent->comment }}</td>
                        <td class="border">{{ $rent->created_at }}</td>
                        {{-- <td class="border">{{ $rent->confirmed == 1 ? }} <i class="fas fa-check-circle"></i> {{ : }} <i class="fas fa-times-circle"></i> </td> --}}
                        @if ($rent->confirmed == 1)
                            <td class="border text-center text-success"><i class="fas fa-check-circle fa-2x"></i></td>
                        @else
                            <td class="border text-center text-danger"><i class="fas fa-times-circle fa-2x"></i></td>
                        @endif

                        <td class="border">
                            <a href="/delete_order/{{ $rent->id }}"><i class="far fa-trash-alt text-danger"></i></a>
                            @if ($rent->confirmed == 0)
                                <a href="/edit_order/{{ $rent->id }}"><i class="far fa-edit text-primary"></i></a>
                            @endif   
                        </td>
                    </tr>
                @endforeach
